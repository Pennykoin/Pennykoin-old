...
const uint64_t CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX       = 136; // addresses start with "XRY1"
...
const uint64_t POINT                                         = UINT64_C(1); // pow(10, 3)
const uint64_t COIN                                          = UINT64_C(100); // pow(10, 6)
const uint64_t MINIMUM_FEE                                   = UINT64_C(1); // pow(10, 4)
const uint64_t DEFAULT_DUST_THRESHOLD                        = UINT64_C(1); // pow(10, 4)
...
const uint32_t DEPOSIT_MAX_TERM                              = 43800; // ~1 year
...
const uint32_t END_MULTIPLIER_BLOCK                          = 1; // block at which the multiplier ceases to be applied
