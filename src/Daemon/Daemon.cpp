...
// 	std::cout << "Modify this line into your PK configuration file as is:  " << std::endl;
// 	std::cout << "const char GENESIS_COINBASE_TX_HEX[] = \"" << tx_hex << "\";" << std::endl;
//   }
//   return;
// }
...
void renameDataDir() {
	std::string PKDir = Tools::getDefaultDataDirectory();
	boost::filesystem::path PKDirPath(PKDir);
	if (boost::filesystem::exists(PKDirPath)) {
		return;
	}

	std::string dataDirPrefix = PKDir.substr(0, PKDir.size() + 1 - sizeof(CRYPTONOTE_NAME));

	boost::filesystem::path cediDirPath(dataDirPrefix + "BXC");
	if (boost::filesystem::exists(cediDirPath)) {
		boost::filesystem::rename(cediDirPath, PKDirPath);
	}
	else {
		boost::filesystem::path BcediDirPath(dataDirPrefix + "Bcedi");
		if (boost::filesystem::exists(boost::filesystem::path(BcediDirPath))) {
			boost::filesystem::rename(BcediDirPath, PKDirPath);
		}
	}
}
